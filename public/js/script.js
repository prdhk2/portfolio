document.addEventListener('DOMContentLoaded', function() {
        ...
        createProjectCard: (project) => {
            ...
            card.innerHTML = `
                <div class="pc-body">
                    ...
                    <p class="pc-desc">${utils.truncateText(project.description)}</p>
                    <div class="pc-links">
                        <a class="view" href="${project.url || '#'}" target="_blank" rel="noopener">View Project</a>
                        ${project.code ? `<a class="code" href="${project.code}" target="_blank" rel="noopener">Source Code</a>` : ''}
                    </div>
                </div>
            `;
            return card;
    ...
    const projectsManager = {
        async loadProjects(tech) {
            ...
            // Show loading state
            elements.projectsGrid.innerHTML = '<div class="loading">Loading projects...</div>';
            elements.projectsSection.hidden = false;

            try {
                // Try to fetch from server first
                const response = await fetch(`/projects?tech=${encodeURIComponent(tech)}`, {
                    headers: { 
    ...
    const eventHandlers = {
        ...
        handleOutsideClick: (e) => {
            ...
            const isClickInsideProjects = elements.projectsSection.contains(e.target);
    ...
    function initEventListeners() {
        ...
        elements.skillsGrid.addEventListener('click', eventHandlers.handleSkillsClick);
